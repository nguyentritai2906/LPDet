...
def IOU(tl1, br1, tl2, br2):
    ...
    intersection_wh = np.maximum(
        np.minimum(br1, br2) - np.maximum(tl1, tl2), 0)
...
def getRectPts(tlx, tly, brx, bry):
    return np.matrix(
        [[tlx, brx, brx, tlx], [tly, tly, bry, bry], [1, 1, 1, 1]],
        dtype=float)
...
# Reconstruct from predict value to LP, cut from original image to LP, label...
def reconstruct(I, Iresized, Yr, lp_threshold):  # noqa: E741
    ...
    def base(vx, vy):
        return np.matrix([[-vx, -vy, 1], [vx, -vy, 1], [vx, vy, 1],
                          [-vx, vy, 1]]).T
    ...
    out_size, lp_type = (two_lines, 2) if (
        (final_labels_frontal[0].wh()[0] /
         final_labels_frontal[0].wh()[1]) < 1.7) else (one_line, 1)
    ...
    if len(final_labels):
        ...
        for _, label in enumerate(final_labels):
            ...
            ptsh = np.concatenate((label.pts * getWH(I.shape).reshape(
                (2, 1)), np.ones((1, 4))))
...
def detect_lp(model, I, max_dim, lp_threshold):  # noqa: E741
    ...
    # Return: Label, Image tensor, LP type (1: Rectangle, 2: Square)
